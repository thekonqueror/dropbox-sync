...
#
# Updated by thekonqueror for Oauth 2.0 and multi-user access
...
API_UPLOAD_URL="https://api-content.dropbox.com/1/files_put/auto"
API_DOWNLOAD_URL="https://api-content.dropbox.com/1/files/auto"
...
function usage
{
    echo -e "Dropbox Uploader v$VERSION - theKonqueror"
...
function db_stat
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" -H "Authorization: Bearer $TOKEN" "$API_METADATA_URL/$(urlencode "$FILE")" 2> /dev/null
    check_http_response
...
function db_simple_upload_file
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES $CURL_PARAMETERS -i --globoff -o "$RESPONSE_FILE" --upload-file "$FILE_SRC" -H "Authorization: Bearer $TOKEN" "$API_UPLOAD_URL/$(urlencode "$FILE_DST")"
    check_http_response
...
function db_chunked_upload_file
{
    ...
    while ([[ $OFFSET != $FILE_SIZE ]]); do
        ...
        #Uploading the chunk...
        $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --upload-file "$CHUNK_FILE" -H "Authorization: Bearer $TOKEN" "$API_CHUNKED_UPLOAD_URL?$CHUNK_PARAMS" 2> /dev/null
        check_http_response
    ...
    while (true); do
        ...
        $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "upload_id=$UPLOAD_ID"  -H "Authorization: Bearer $TOKEN" "$API_CHUNKED_UPLOAD_COMMIT_URL/$(urlencode "$FILE_DST")" 2> /dev/null
        check_http_response
...
{
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" -H "Authorization: Bearer $TOKEN"  "$API_INFO_URL" 2> /dev/null
    check_http_response
...
function db_download_file
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES $CURL_PARAMETERS --globoff -D "$RESPONSE_FILE" -H "Authorization: Bearer $TOKEN" -o "$FILE_DST" "$API_DOWNLOAD_URL/$(urlencode "$FILE_SRC")"
    check_http_response
...
function db_delete
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -H "Authorization: Bearer $TOKEN" -o "$RESPONSE_FILE" --data "path=$(urlencode "$FILE_DST")" "$API_DELETE_URL" 2> /dev/null
    check_http_response
...
function db_move
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -H "Authorization: Bearer $TOKEN" -o "$RESPONSE_FILE" --data "from_path=$(urlencode "$FILE_SRC")&to_path=$(urlencode "$FILE_DST")" "$API_MOVE_URL" 2> /dev/null
    check_http_response
...
function db_copy
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -H "Authorization: Bearer $TOKEN" -o "$RESPONSE_FILE" --data "from_path=$(urlencode "$FILE_SRC")&to_path=$(urlencode "$FILE_DST")" "$API_COPY_URL" 2> /dev/null
    check_http_response
...
function db_mkdir
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -H "Authorization: Bearer $TOKEN" -o "$RESPONSE_FILE" --data "path=$(urlencode "$DIR_DST")&root=auto" "$API_MKDIR_URL" 2> /dev/null
    check_http_response
...
function db_list
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" -H "Authorization: Bearer $TOKEN" "$API_METADATA_URL/$(urlencode "$DIR_DST")" 2> /dev/null
    check_http_response
...
function db_share
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -H "Authorization: Bearer $TOKEN" -o "$RESPONSE_FILE" "$API_SHARES_URL/$(urlencode "$FILE_DST")" 2> /dev/null
    check_http_response
...
#### SETUP  ####
################
CONFIG_FILE=/home/$APPNAME/.dropbox
...
case $COMMAND in
    ...
    upload)
        ...
        for (( i=$OPTIND+1; i<$#; i++ )); do
            FILE_SRC=${@:$i:1}
            db_upload "/home/$APPNAME/public_html/$FILE_SRC" "/$APPNAME.myapp.com/$FILE_DST"
        done
    ...
    download)
        ...
        db_download "/$APPNAME.myapp.com/$FILE_SRC" "/home/$APPNAME/public_html/$FILE_DST"
